...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'ARAH.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }

		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'ARAH.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'ARAH.Client.ProtectedToken'
