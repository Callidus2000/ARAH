...
$parentModule = 'ARAH'
if (-not $ModuleName) { $ModuleName = 'ARAH.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ARAH'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ARAH.psd1"
$prereqHash = @{
